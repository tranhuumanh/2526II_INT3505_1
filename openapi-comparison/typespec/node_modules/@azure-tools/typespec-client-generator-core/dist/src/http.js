...
import { addEncodeInfo, getClientTypeWithDiagnostics, getSdkConstant, getSdkModelPropertyTypeBase, getTypeSpecBuiltInType, isReadOnly, } from "./types.js";
function buildSdkStreamMetadata(context, tspStreamMetadata, operation) {
    const diagnostics = createDiagnosticCollector();
    const bodyType = diagnostics.pipe(getClientTypeWithDiagnostics(context, tspStreamMetadata.bodyType, operation));
    const originalType = diagnostics.pipe(getClientTypeWithDiagnostics(context, tspStreamMetadata.originalType, operation));
    const streamType = diagnostics.pipe(getClientTypeWithDiagnostics(context, tspStreamMetadata.streamType, operation));
...
export function getSdkHttpOperation(context, httpOperation, methodParameters, client) {
    const tk = $(context.program);
    const diagnostics = createDiagnosticCollector();
    const { responses, exceptions } = diagnostics.pipe(getSdkHttpResponseAndExceptions(context, httpOperation, client));
    if (getResponseAsBool(context, httpOperation.operation)) {
        ...
        if (fourOFourResponse) {
            ...
            responses.push({
                ...
                apiVersions: getAvailableApiVersions(context, httpOperation.operation, getActualClientType(client.__raw)),
                headers: [],
...
function getSdkHttpParameters(context, httpOperation, methodParameters, responseBody) {
    ...
    const methodParametersMap = new Map();
    ...
    retval.parameters = filteredParams.map((x) => diagnostics.pipe(getSdkHttpParameter(context, x.param, httpOperation.operation, x, x.type)));
    ...
    if (tspBody) {
        ...
        if (retval.bodyParam) {
            retval.bodyParam.methodParameterSegments = diagnostics.pipe(getMethodParameterSegments(context, httpOperation.operation, methodParameters, methodParametersMap, retval.bodyParam));
            // Derive correspondingMethodParams from methodParameterSegments (last element of each path)
            // eslint-disable-next-line @typescript-eslint/no-deprecated
            retval.bodyParam.correspondingMethodParams = retval.bodyParam.methodParameterSegments.map((segment) => segment[segment.length - 1]);
            ...
            if (requestStreamMeta) {
                ...
                retval.bodyParam.streamMetadata = diagnostics.pipe(buildSdkStreamMetadata(context, requestStreamMeta, httpOperation.operation));
                // eslint-disable-next-line @typescript-eslint/no-deprecated
    ...
    if (retval.bodyParam && !headerParams.some((h) => isContentTypeHeader(h))) {
        ...
        let methodParameter = methodParameters.find((m) => m.name === "contentType");
        if (!methodParameter) {
            methodParameter = {
                ...contentTypeBase,
                kind: "method",
    ...
    if (responseBody && !headerParams.some((h) => isAcceptHeader(h))) {
        ...
        let methodParameter = methodParameters.find((m) => m.name === "accept");
        if (!methodParameter) {
            methodParameter = {
                ...acceptBase,
                kind: "method",
    ...
    for (const param of retval.parameters) {
        if (param.methodParameterSegments.length > 0)
            continue;
        param.methodParameterSegments = diagnostics.pipe(getMethodParameterSegments(context, httpOperation.operation, methodParameters, methodParametersMap, param));
        // Derive correspondingMethodParams from methodParameterSegments (last element of each path)
        // eslint-disable-next-line @typescript-eslint/no-deprecated
        param.correspondingMethodParams = param.methodParameterSegments.map((segment) => segment[segment.length - 1]);
    }
    return diagnostics.wrap(retval);
}
...
export function getSdkHttpParameter(context, param, operation, httpParam, location) {
    ...
    return diagnostics.wrap({
        ...headerQueryBase,
        kind: "header",
        serializedName: getHeaderFieldName(program, param) ??
            httpParam?.name ??
            base.name,
    });
}
function getSdkHttpResponseAndExceptions(context, httpOperation, client) {
    const tk = $(context.program);
    const diagnostics = createDiagnosticCollector();
    const responses = [];
    const exceptions = [];
        ...
        for (const innerResponse of response.responses) {
            ...
            if (innerResponse.body && !isNeverOrVoidType(innerResponse.body.type)) {
                ...
                if (responseStreamMeta) {
                    ...
                    streamMetadata = diagnostics.pipe(buildSdkStreamMetadata(context, responseStreamMeta, httpOperation.operation));
                }
                else {
 ...
 */
export function getMethodParameterSegments(context, operation, methodParameters, methodParametersMap, serviceParam) {
    const diagnostics = createDiagnosticCollector();
    if (serviceParam.onClient) {
        ...
        if (clientParams && serviceParam.isApiVersionParam && serviceParam.onClient) {
            ...
            if (existingApiVersion)
                return diagnostics.wrap([[existingApiVersion]]);
        }
    ...
    const directMappingPath = findMappingWithPath(context, methodParameters, methodParametersMap, serviceParam);
    ...
    if (serviceParam.kind === "body" && serviceParam.type.kind === "model") {
        ...
        for (const serviceParamProp of serviceParam.type.properties) {
            const propertyMappingPath = findMappingWithPath(context, methodParameters, methodParametersMap, serviceParamProp);
 ...
 */
function findMappingWithPath(context, methodParameters, methodParametersMap, serviceParam) {
...
function buildPathFromParentMap(node, parentMap) {
    const path = [];
    let current = node;
...
function filterOutReadOnlyParameters(methodParameters) {
