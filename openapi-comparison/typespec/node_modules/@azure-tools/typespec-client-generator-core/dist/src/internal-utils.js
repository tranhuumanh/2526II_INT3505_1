import { FinalStateValue, getLroMetadata, isPreviewVersion, } from "@azure-tools/typespec-azure-core";
import { compilerAssert, createDiagnosticCollector, getDeprecationDetails, getDoc, getLifecycleVisibilityEnum, getNamespaceFullName, getSummary, getVisibilityForClass, ignoreDiagnostics, isNeverType, isNullType, isTemplateDeclaration, isVoidType, listServices, } from "@typespec/compiler";
import { unsafe_mutateSubgraphWithNamespace, unsafe_Realm, } from "@typespec/compiler/experimental";
import { $ } from "@typespec/compiler/typekit";
import { getHeaderFieldOptions, getPathParamOptions, getQueryParamOptions, isHeader, isPathParam, isQueryParam, } from "@typespec/http";
import { getAddedOnVersions, getRemovedOnVersions, getVersioningMutators, getVersions, } from "@typespec/versioning";
import assert from "assert";
import { getAlternateType, getClientDocExplicit, getClientLocation, getLegacyHierarchyBuilding, getMarkAsLro, getOverriddenClientMethod, getParamAlias, getUsageOverride, } from "./decorators.js";
import { createDiagnostic, createStateSymbol, reportDiagnostic } from "./lib.js";
import { getSdkBasicServiceMethod } from "./methods.js";
import { getCrossLanguageDefinitionId, getHttpOperationWithCache, isApiVersion, } from "./public-utils.js";
...
export function hasExplicitClientOrOperationGroup(context) {
...
export function listScopedDecoratorData(context, key, languageScope) {
...
export function getScopedDecoratorData(context, key, target, languageScope) {
 ...
 */
function isScopeApplicable(scopeArg, emitterName) {
 ...
 */
export function findServiceForOperation(services, operation) {
 ...
 */
export function updateWithApiVersionInformation(context, type, client, operation) {
    ...
    if (operation) {
        ...
        return {
            isApiVersionParam,
            clientDefaultValue: packageVersions.length > 0 ? packageVersions[packageVersions.length - 1] : undefined,
        };
    }
...
export function filterPreviewVersion(context, sdkVersionsEnum, defaultApiVersion) {
 ...
 * @returns
 */
export function getHttpBodyType(httpBody) {
...
export function getCorrespondingClientParam(context, type, operation) {
    const clientParams = [];
...
export function hasNoneVisibility(context, type) {
...
function listAllNamespaces(context, namespace, retval) {
    if (!retval) {
        retval = [];
    }
    if (retval.includes(namespace))
        return retval;
...
export function listAllUserDefinedNamespaces(context) {
    return listAllNamespaces(context, context.getMutatedGlobalNamespace()).filter((ns) => $(context.program).type.isUserDefined(ns));
}
export function findRootSourceProperty(property) {
    while (property.sourceProperty) {
        property = property.sourceProperty;
    }
    return property;
}
export function getStreamAsBytes(context, type) {
    const diagnostics = createDiagnosticCollector();
    const unknownType = {
...
export function handleVersioningMutationForGlobalNamespace(context) {
    ...
    if (context.apiVersion === "all")
        return globalNamespace;
    const explicitClientNamespaces = [];
    const explicitServices = new Set();
    listScopedDecoratorData(context, clientKey).forEach((v, k) => {
        ...
        if (!unsafe_Realm.realmForType.has(k)) {
            const sdkClient = v;
            explicitClientNamespaces.push(k);
    ...
    if (explicitClientNamespaces.length === 0 || explicitServices.size === 1) {
        const serviceNamespace = explicitClientNamespaces.length === 0
            ? services[0].type
        ...
        mutator = getVersioningMutator(context, serviceNamespace, versionsValues[versionsValues.length - 1]);
    }
...
export function resolveDuplicateGenearatedName(context, type, createName) {
...
export function resolveConflictGeneratedName(context) {
...
export function getClientDoc(context, target) {
...
export function compareModelProperties(program, modelPropA, modelPropB) {
    ...
    if (aHasHttpKind && bHasHttpKind) {
        if (aIsQuery !== bIsQuery || aIsHeader !== bIsHeader || aIsPath !== bIsPath)
            return false;
    }
    if (aIsQuery &&
        bIsQuery &&
...
export function* filterMapValuesIterator(iterator, predicate) {
 ...
 */
export function findEntriesWithTarget(context, stateKey, targetValue, sourceKind) {
    const results = [];
 ...
 */
export function getTcgcLroMetadata(context, operation, client) {
    ...
    if (getMarkAsLro(context, operation)) {
        ...
        switch (sdkMethodResponseType.kind) {
            case "nullable":
                returnType = sdkMethodResponseType.type.__raw;
                break;
            case "model":
                returnType = sdkMethodResponseType.__raw;
                break;
            default:
                throw new Error(`LRO method ${operation.name} with @markAsLro must have a model return type.`);
        }
        return {
            operation,
...
export function getActualClientType(client) {
    if (client.kind === "SdkClient")
        return client.type;
    if (client.type)
        return client.type;
...
export function isSameServers(left, right) {
...
export function isSameAuth(left, right) {
                    ...
                    for (let k = 0; k < leftScheme.flows.length; k++) {
                        ...
                        switch (leftFlow.type) {
                            case "authorizationCode":
                                ...
                                if (leftFlow.authorizationUrl !== rightFlow.authorizationUrl ||
                            ...
                            case "clientCredentials":
                                ...
                                if (leftFlow.tokenUrl !== rightFlow.tokenUrl ||
                                    leftFlow.refreshUrl !== rightFlow.refreshUrl) {
                                    return false;
                                }
                                break;
...
export function isTypeNeedsHandling(context, type) {
    return ((context.__mutatedRealm === undefined && !unsafe_Realm.realmForType.has(type)) ||
        (context.__mutatedRealm !== undefined && context.__mutatedRealm.hasType(type)));
}
export function listOrphanTypes(context) {
    if (context.__orphanTypesCache)
    ...
    for (const currNamespace of userDefinedNamespaces) {
        ...
        while (currentIndex < namespaces.length) {
            ...
            for (const enumType of namespace.enums.values()) {
                if (!getUsageOverride(context, enumType))
                    continue;
                result.push(enumType);
            }
            // orphan unions
            for (const unionType of namespace.unions.values()) {
                if (isTemplateDeclaration(unionType))
                    continue;
                if (!getUsageOverride(context, unionType))
                    continue;
