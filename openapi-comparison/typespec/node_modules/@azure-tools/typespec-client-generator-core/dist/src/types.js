...
function updateTypesFromOperation(context, operation) {
    ...
    for (const param of (overriddenClientMethod ?? operation).parameters.properties.values()) {
        ...
        if (param.type.kind === "Model" && isStream(program, param.type)) {
            ...
            if (streamOf && generateConvenient) {
                const sdkStreamType = diagnostics.pipe(getClientTypeWithDiagnostics(context, streamOf, operation));
    ...
    for (const param of httpOperation.parameters.parameters) {
        if (isNeverOrVoidType(param.param.type))
            continue;
        // skip parameters that are out of scope
        if (!isInScope(context, param.param))
            continue;
    ...
    if (requestStreamMeta && generateConvenient) {
        const sdkStreamType = diagnostics.pipe(getClientTypeWithDiagnostics(context, requestStreamMeta.streamType, operation));
        diagnostics.pipe(updateUsageOrAccess(context, UsageFlags.Input, sdkStreamType));
    ...
    for (const response of httpOperation.responses) {
        for (const innerResponse of response.responses) {
            ...
            if (responseStreamMeta && generateConvenient) {
                const sdkStreamType = diagnostics.pipe(getClientTypeWithDiagnostics(context, responseStreamMeta.streamType, operation));
...
function handleServiceOrphanTypes(context) {
    const diagnostics = createDiagnosticCollector();
