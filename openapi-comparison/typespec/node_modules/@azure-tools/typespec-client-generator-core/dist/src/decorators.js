...
import { compilerAssert, getDiscriminator, getNamespaceFullName, ignoreDiagnostics, isErrorModel, isList, isNumeric, isService, isTemplateDeclaration, } from "@typespec/compiler";
...
import { AllScopes, clientKey, clientLocationKey, clientNameKey, clientNamespaceKey, compareModelProperties, findEntriesWithTarget, findRootSourceProperty, getScopedDecoratorData, hasExplicitClientOrOperationGroup, isSameAuth, isSameServers, legacyHierarchyBuildingKey, listAllUserDefinedNamespaces, negationScopesKey, omitOperation, operationGroupKey, overrideKey, parseScopes, scopeKey, usageKey, } from "./internal-utils.js";
...
export const $client = (context, target, options, scope) => {
    ...
    let services;
    const serviceConfig = options?.kind === "Model" ? options?.properties.get("service")?.type : undefined;
    if (serviceConfig?.kind === "Namespace") {
        services = [serviceConfig];
    }
    else if (serviceConfig?.kind === "Tuple" &&
        ...
        if (!target.decorators.some((d) => d.definition?.name === "@useDependency" &&
...
function getConvenientOrProtocolValue(context, key, entity) {
    ...
    let currentNamespace = entity.namespace;
...
function collectParams(program, properties, params = []) {
...
export const $override = (context, original, override, scope) => {
    ...
    const originalParams = collectParams(context.program, original.parameters.properties).sort((a, b) => a.name.localeCompare(b.name));
    const overrideParams = collectParams(context.program, override.parameters.properties).sort((a, b) => a.name.localeCompare(b.name));
    // Check if the sorted parameter names arrays are equal, omit optional parameters
    let parametersMatch = true;
    let checkParameter = undefined;
    let index = 0;
    for (const originalParam of originalParams) {
        ...
        overrideParam.decorators
            .filter((d) => d.definition?.name === "@alternateType" &&
            getNamespaceFullName(d.definition?.namespace) === namespace)
            .map((d) => context.call($alternateType, originalParam, d.args[0].value, d.args[1]?.jsValue));
        index++;
    }
    if (!parametersMatch) {
        reportDiagnostic(context.program, {
...
function isExternalType(model) {
    ...
    const hasIdentity = properties.some((prop) => prop.name === "identity" && prop.type.kind === "String");
    if (!hasIdentity) {
        return false;
    }
    ...
    const validProps = otherProps.every((prop) => (prop.name === "package" || prop.name === "minVersion") && prop.type.kind === "String");
    return validProps;
}
...
export function getAlternateType(context, source) {
    const retval = getScopedDecoratorData(context, alternateTypeKey, source);
...
export const $clientInitialization = (context, target, options, scope) => {
    if (options.kind === "Model") {
        if (options.properties.get("initializedBy")) {
            ...
            const isValidValue = (value) => value === 4 || value === 1 || value === 2;
            if (value.kind === "EnumMember") {
                if (typeof value.value !== "number" || !isValidValue(value.value)) {
                    reportDiagnostic(context.program, {
...
export function getClientInitializationOptions(context, entity) {
    ...
    while (currEntity) {
        const movedParameters = findEntriesWithTarget(context, clientLocationKey, currEntity, "ModelProperty");
...
function getNamespaceFullNameWithOverride(context, namespace) {
    ...
    if (isOverridden) {
        ...
        if (userDefinedNamespace && context.namespaceFlag) {
            ...
            if (context.namespaceFlag.startsWith(userDefinedNamespace.name) &&
    ...
    if (source.kind === "Operation") {
        ...
        if (typeof target !== "string" &&
            ...
            const clientInitializationParams = target.decorators
                ...
                .filter((a) => a.entityKind === "Type" && a.kind === "Model")
 ...
 */
export function getClientLocation(context, input) {
...
function isPropertySuperset(program, target, value) {
    const conflicts = [];
...
export const $legacyHierarchyBuilding = (context, target, value, scope) => {
...
export function getLegacyHierarchyBuilding(context, target) {
...
export const $markAsLro = (context, target, scope) => {
    const httpOperation = ignoreDiagnostics(getHttpOperation(context.program, target));
    const hasModelResponse = httpOperation.responses.filter((r) => r.type?.kind === "Model" && !(r.statusCodes === "*" || isErrorModel(context.program, r.type)))[0];
    if (!hasModelResponse) {
        reportDiagnostic(context.program, {
...
export function getMarkAsLro(context, entity) {
    return getScopedDecoratorData(context, markAsLroKey, entity) ?? false;
}
const markAsPageableKey = createStateSymbol("markAsPageable");
export const $markAsPageable = (context, target, scope) => {
    const httpOperation = ignoreDiagnostics(getHttpOperation(context.program, target));
    const modelResponse = httpOperation.responses.filter((r) => r.type?.kind === "Model" && !(r.statusCodes === "*" || isErrorModel(context.program, r.type)))[0];
    if (!modelResponse) {
        reportDiagnostic(context.program, {
    ...
    const responseType = getRealResponseModel(context.program, modelResponse.type);
...
export const $nextLinkVerb = (context, target, verb, scope) => {
    compilerAssert(verb.kind === "String" && (verb.value === "POST" || verb.value === "GET"), "@nextLinkVerb decorator only supports 'POST' or 'GET' string literal values.");
 ...
 */
export function getNextLinkVerb(context, entity) {
...
export const $clientDefaultValue = (context, target, value, scope) => {
    const actualValue = isNumeric(value) ? value.asNumber() : value;
    setScopedDecoratorData(context, $clientDefaultValue, clientDefaultValueKey, target, actualValue, scope);
};
/**
 ...
 */
export function getClientDefaultValue(context, entity) {
 ...
 */
export const $clientOption = (context, target, name, value, scope) => {
 ...
 */
export function getClientOptionValue(context, target, optionName) {
    // Check operation directly
    const opOption = getScopedDecoratorData(context, clientOptionKey, target);
    if (opOption?.name === optionName) {
        return opOption.value;
    }
    // Check interface if operation is in one
    if (target.interface) {
        const ifaceOption = getScopedDecoratorData(context, clientOptionKey, target.interface);
    ...
    while (ns) {
        const nsOption = getScopedDecoratorData(context, clientOptionKey, ns);
 ...
 */
export function shouldOmitSlashFromEmptyRoute(context, target) {
