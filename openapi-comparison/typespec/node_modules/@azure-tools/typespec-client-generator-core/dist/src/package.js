import { createDiagnosticCollector, getNamespaceFullName, ignoreDiagnostics, } from "@typespec/compiler";
...
import { filterApiVersionsWithDecorators, getActualClientType, getTypeDecorators, } from "./internal-utils.js";
...
export function createSdkPackage(context) {
    ...
    for (const [namespace, versionList] of versions.entries()) {
        ...
        if (latestVersion) {
            ...
            const versionValue = context.apiVersion === "all" && versions.size === 1 ? "all" : latestVersion;
    ...
    const sdkPackage = {
        ...
        metadata: {
            apiVersion: context.apiVersion === "all" && versions.size === 1
                ? "all"
                : versions.size === 1
...
function populateApiVersionInformation(context) {
    ...
    for (const client of context.__rawClientsOperationGroupsCache.values()) {
        ...
        if (client.services.length > 1) {
            ...
            const versions = filterApiVersionsWithDecorators(context, clientType, packageVersions.get(client.services[0]) || []);
