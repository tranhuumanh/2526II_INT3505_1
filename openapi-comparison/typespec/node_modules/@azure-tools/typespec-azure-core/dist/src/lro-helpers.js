...
function createLroMetadata(program, operation, context) {
    ...
    const logicalPathName = context.finalStep?.kind === "pollingSuccessProperty"
        ...
        : undefined;
    let finalResult = model.kind === "Model" ||
        model.kind === "Scalar" ||
        (model.kind === "Intrinsic" && !isVoidType(model))
        ? model
        : "void";
    let finalEnvelopeResult = model.kind === "Model" ||
        model.kind === "Scalar" ||
        (model.kind === "Intrinsic" && !isVoidType(model))
        ? model
        : "void";
    ...
    return {
        operation: operation,
        logicalResult: model.kind === "Intrinsic" || model.kind === "Scalar"
...
function getStatusMonitorInfo(program, modelOrLink, pollingOverride) {
    ...
    if (modelOrLink.kind === "link") {
        ...
        if (statusMonitorType === undefined ||
...
function createStatusMonitorPollingData(data) {
    function getModelOrScalar(property) {
