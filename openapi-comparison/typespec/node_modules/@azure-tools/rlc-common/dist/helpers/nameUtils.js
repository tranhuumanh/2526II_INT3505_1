...
exports.CasingConvention = exports.ReservedModelNames = exports.NameType = void 0;
...
function guardReservedNames(name, nameType, customReservedNames = []) {
    ...
    return [...exports.ReservedModelNames, ...customReservedNames]
        .filter((r) => r.reservedFor.includes(nameType))
        .find((r) => r.name === name.toLowerCase())
        ? `${prefix}${name}${suffix}`
        : name;
}
function getAffix(nameType) {
    switch (nameType) {
        case NameType.File:
...
function normalizeName(name, nameType, optionsOrShouldGuard, optionalCustomReservedNames, optionalCasingOverride) {
    var _a;
    let shouldGuard, customReservedNames, casingOverride, numberPrefixOverride;
    if (typeof optionsOrShouldGuard === "boolean") {
        shouldGuard = optionsOrShouldGuard;
        customReservedNames = optionalCustomReservedNames !== null && optionalCustomReservedNames !== void 0 ? optionalCustomReservedNames : [];
        casingOverride = optionalCasingOverride;
    }
    else {
        shouldGuard = optionsOrShouldGuard === null || optionsOrShouldGuard === void 0 ? void 0 : optionsOrShouldGuard.shouldGuard;
        customReservedNames = (_a = optionsOrShouldGuard === null || optionsOrShouldGuard === void 0 ? void 0 : optionsOrShouldGuard.customReservedNames) !== null && _a !== void 0 ? _a : [];
        casingOverride = optionsOrShouldGuard === null || optionsOrShouldGuard === void 0 ? void 0 : optionsOrShouldGuard.casingOverride;
        numberPrefixOverride = optionsOrShouldGuard === null || optionsOrShouldGuard === void 0 ? void 0 : optionsOrShouldGuard.numberPrefixOverride;
    }
...
function fixLeadingNumber(name, nameType, prefix = "_") {
...
function isFullyUpperCase(identifier, maxUppercasePreserve = 3) {
    const len = identifier.length;
    if (len > 1) {
        if (len <= maxUppercasePreserve &&
...
function toCasing(str, casing, keepConsistent = false) {
    const firstChar = casing === CasingConvention.Pascal
        ? str.charAt(0).toUpperCase()
        : str.charAt(0).toLowerCase();
    const allLowerCases = casing !== CasingConvention.Pascal &&
        keepConsistent &&
        str.toUpperCase() === str;
