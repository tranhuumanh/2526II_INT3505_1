...
import { getImportModuleName, getResponseBaseName, getResponseTypeName } from "./helpers/nameConstructors.js";
...
export function buildResponseTypes(model) {
    ...
    if ((model.importInfo.internalImports.response?.importsSet?.size ?? 0) > 0) {
        const modelNamedImports = Array.from(model.importInfo.internalImports.response.importsSet).filter((modelName) => {
