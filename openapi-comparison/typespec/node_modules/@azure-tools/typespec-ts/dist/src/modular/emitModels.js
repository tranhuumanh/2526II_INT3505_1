...
import { fixLeadingNumber, NameType, normalizeName } from "@azure-tools/rlc-common";
import { UsageFlags, isPagedResultModel, isReadOnly, listAllServiceNamespaces } from "@azure-tools/typespec-client-generator-core";
import { getExternalModel, getModelExpression, getMultipartFileTypeExpression } from "./type-expressions/get-model-expression.js";
...
import { buildXmlModelSerializer, buildXmlModelDeserializer, buildXmlObjectModelSerializer, buildXmlObjectModelDeserializer, hasXmlSerialization } from "./serialization/buildXmlSerializerFunction.js";
...
import { getAllDiscriminatedValues, getPropertyWithOverrides, isDiscriminatedUnion } from "./serialization/serializeUtils.js";
...
import { emitQueue, flattenPropertyModelMap, getAllOperationsFromClient } from "../framework/hooks/sdkTypes.js";
import { getAllAncestors, getAllProperties, buildNonModelResponseTypeDeclaration, checkWrapNonModelReturn } from "./helpers/operationHelpers.js";
...
/* eslint-disable @typescript-eslint/explicit-module-boundary-types */`);
    ...
    for (const modelFile of modelFiles) {
        if (modelFile.getInterfaces().length === 0 &&
 ...
 */
export function emitNonModelResponseTypes(context, { sourceRoot }) {
    const outputProject = useContext("outputProject");
...
function emitType(context, type, sourceFile) {
    ...
    if (type.kind === "model") {
        ...
        if (apiVersionEnumOnly && ((_b = context.rlcOptions) === null || _b === void 0 ? void 0 : _b.isMultiService)) {
            return;
        }
        ...
        const [enumType, knownValuesEnum] = buildEnumTypes(context, type, isExtensibleEnum(context, type));
...
export function getModelsPath(sourceRoot, modelNamespace = []) {
    return path.join(...[
        sourceRoot,
        "models",
...
export function getModelNamespaces(context, model) {
    var _a, _b, _c, _d, _e, _f, _g;
    if (model.kind === "model" ||
        model.kind === "enum" ||
        model.kind === "union") {
        if (((_a = model.namespace) !== null && _a !== void 0 ? _a : "").startsWith("Azure.ResourceManager") ||
            ((_b = model.namespace) !== null && _b !== void 0 ? _b : "").startsWith("Azure.Core") ||
            ((_c = model.crossLanguageDefinitionId) !== null && _c !== void 0 ? _c : "").startsWith("TypeSpec.Rest.Resource") ||
            ((_d = model.crossLanguageDefinitionId) !== null && _d !== void 0 ? _d : "") === "TypeSpec.Http.File" // filter out the TypeSpec.Http.File model similar like what java does here https://github.com/microsoft/typespec/blob/main/packages/http-client-java/emitter/src/code-model-builder.ts#L2589
        ) {
            return [];
        }
        ...
        if ((_e = context.rlcOptions) === null || _e === void 0 ? void 0 : _e.isMultiService) {
            return segments;
        }
        const allServiceNamespaces = (_f = context.allServiceNamespaces) !== null && _f !== void 0 ? _f : listAllServiceNamespaces(context);
        const deepestNamespace = getNamespaceFullName(allServiceNamespaces[0]);
        const rootNamespace = (_g = deepestNamespace.split(".")) !== null && _g !== void 0 ? _g : [];
 ...
 */
function isNestedInXmlModel(targetModel) {
    for (const type of emitQueue) {
 ...
 */
function getModelAndAncestorProperties(type) {
    const properties = [];
    let current = type;
    while (current) {
...
function addSerializationFunctions(context, typeOrProperty, sourceFile, skipDiscriminatedUnionSuffix = false) {
    const options = {
        nameOnly: false,
        skipDiscriminatedUnionSuffix
    };
    // Add JSON serializers
    const serializationFunction = typeOrProperty.kind === "property"
    ...
    const deserializationFunction = typeOrProperty.kind === "property"
    ...
    if (typeOrProperty.kind === "model" && hasXmlSerialization(typeOrProperty)) {
        ...
        if (isNestedInXmlModel(typeOrProperty)) {
            ...
            const xmlObjectSerializationFunction = buildXmlObjectModelSerializer(context, typeOrProperty, options);
            if (xmlObjectSerializationFunction &&
            ...
            const xmlObjectDeserializationFunction = buildXmlObjectModelDeserializer(context, typeOrProperty, options);
            if (xmlObjectDeserializationFunction &&
...
function buildNullableType(context, type) {
    var _a;
    const nullableDeclaration = {
        kind: StructureKind.TypeAlias,
        name: normalizeModelName(context, type),
        isExported: true,
    ...
    nullableDeclaration.docs = [
        (_a = type.doc) !== null && _a !== void 0 ? _a : `Alias for ${nullableDeclaration.name}`
    ];
    return nullableDeclaration;
}
export function buildEnumTypes(context, type, reportMemberNameDiagnostic = false // if reportMemberNameDiagnostic is true, it will report diagnostic for enum member name
) {
    var _a;
    const rawMembers = type.values.map((value) => emitEnumMember(context, value, reportMemberNameDiagnostic));
    ...
    enumAsUnion.docs =
        isExtensibleEnum(context, type) && type.doc
            ? [(_a = getExtensibleEnumDescription(context, type)) !== null && _a !== void 0 ? _a : docs]
            : [docs];
...
function getExtensibleEnumDescription(context, model) {
 ...
 */
function deduplicateEnumMemberNames(members) {
    var _a;
    const nameCount = new Map();
    for (const member of members) {
        const name = member.name;
        nameCount.set(name, ((_a = nameCount.get(name)) !== null && _a !== void 0 ? _a : 0) + 1);
    }
    const nameCurrentIndex = new Map();
    return members.map((member) => {
        var _a, _b;
        const name = member.name;
        if (((_a = nameCount.get(name)) !== null && _a !== void 0 ? _a : 0) > 1) {
            const index = ((_b = nameCurrentIndex.get(name)) !== null && _b !== void 0 ? _b : 0) + 1;
...
function emitEnumMember(context, member, reportMemberNameDiagnostic = false // if reportMemberNameDiagnostic is true, it will report diagnostic for enum member name
) {
    var _a;
    ...
    let normalizedMemberName = ((_a = context.rlcOptions) === null || _a === void 0 ? void 0 : _a.ignoreEnumMemberNameNormalize)
...
function buildModelInterface(context, type) {
    var _a, _b;
    const flattenPropertySet = new Set();
    ...
    for (const flatten of flattenPropertySet.keys()) {
        const conflictMap = (_a = useContext("sdkTypes").flattenProperties.get(flatten)) === null || _a === void 0 ? void 0 : _a.conflictMap;
        const allProperties = getAllProperties(context, flatten.type, getAllAncestors(flatten.type)).filter((p) => !isMetadata(context.program, p.__raw));
        interfaceStructure.properties.push(...allProperties.map((p) => {
    ...
    if (headerOptions.length > 0) {
        const existingNames = new Set(interfaceStructure.properties.map((p) => p.name));
    ...
    interfaceStructure.docs = [
        (_b = type.doc) !== null && _b !== void 0 ? _b : "model interface " + interfaceStructure.name
    ];
...
function addExtendedDictInfo(context, model, modelInterface) {
    ...
    const additionalPropertiesType = model.additionalProperties
        ...
        : undefined;
    if ((_a = context.rlcOptions) === null || _a === void 0 ? void 0 : _a.compatibilityMode) {
...
export function getAdditionalPropertiesName(context, model) {
    const ancestors = getAllAncestors(model);
    const properties = getAllProperties(context, model, ancestors);
...
export function normalizeModelName(context, type, nameType = NameType.Interface, skipPolymorphicUnionSuffix = false, rawModelName) {
    var _a;
    if (type.kind === "array") {
        if (rawModelName) {
            return `${normalizeModelName(context, type.valueType, nameType, skipPolymorphicUnionSuffix, rawModelName)}Array`;
        }
        return `Array<${normalizeModelName(context, type.valueType, nameType)}>`;
    }
    else if (type.kind === "dict") {
        if (rawModelName) {
            return `${normalizeModelName(context, type.valueType, nameType, skipPolymorphicUnionSuffix, rawModelName)}Record`;
        }
    ...
    const namespacePrefix = ((_a = context.rlcOptions) === null || _a === void 0 ? void 0 : _a.enableModelNamespace)
        ? segments.join("")
        : "";
    const internalModelPrefix = isPagedResultModel(context, type) || type.isGeneratedName ? "_" : "";
...
function buildModelProperty(context, property, model) {
    var _a, _b;
    ...
    if (isDiscriminatedUnion(model) && allDiscriminatorValues.length > 1) {
        ...
        ((_b = property.serializationOptions.multipart) === null || _b === void 0 ? void 0 : _b.isFilePart)) {
