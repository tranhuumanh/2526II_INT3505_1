...
import { buildParameterValueMap, prepareCommonParameters, getDescriptiveName, iterateClientsAndMethods, generateMethodCall, createSourceFile, generateResponseAssertions } from "./helpers/exampleValueHelpers.js";
 ...
 */
async function cleanupTestFolder(dpgContext) {
    var _a, _b;
    const clients = dpgContext.sdkPackage.clients;
    const baseTestFolder = join((_b = (_a = dpgContext.generationPathDetail) === null || _a === void 0 ? void 0 : _a.rootDir) !== null && _b !== void 0 ? _b : "", "test", "generated");
 ...
 */
export async function emitTests(dpgContext) {
    // Clean up the test/generated folder before generating new tests
    await cleanupTestFolder(dpgContext);
    return iterateClientsAndMethods(dpgContext, emitMethodTests);
}
function emitMethodTests(dpgContext, method, options) {
    var _a, _b, _c, _d, _e;
    const examples = (_a = method.operation.examples) !== null && _a !== void 0 ? _a : [];
    if (examples.length === 0) {
        return;
    }
    const methodPrefix = `${(_b = options.classicalMethodPrefix) !== null && _b !== void 0 ? _b : ""} ${(_c = method.oriName) !== null && _c !== void 0 ? _c : method.name}`;
    ...
    const testFunctions = [];
    let clientParamNames = [];
    let clientParameterDefs = [];
    // Create test describe block
    const methodDescription = (_d = method.doc) !== null && _d !== void 0 ? _d : `test ${(_e = method.oriName) !== null && _e !== void 0 ? _e : method.name}`;
    ...
    for (const example of examples) {
        ...
        if (method.response.type === undefined) {
            ...
            const pagingAssertions = generateResponseAssertions(example, "resArray", true // isPaging = true
            );
...
//# sourceMappingURL=emitTests.js.map