...
export function buildClassicalClient(dpgContext, clientMap, emitterOptions) {
    ...
    const clientFile = project.createSourceFile(`${srcPath}/${subfolder && subfolder !== "" ? subfolder + "/" : ""}${normalizeName(classicalClientName, NameType.File)}.ts`);
    ...
    const hasChildClient = client.children &&
        client.children.some((childClient) => {
            return (childClient.clientInitialization.initializedBy &
    ...
    const hasSubscriptionIdParam = classicalParams.some((param) => param.name.toLowerCase() === "subscriptionid");
    const shouldSubscriptionIdOptional = dpgContext.arm &&
    ...
    const paramNames = (contextParams !== null && contextParams !== void 0 ? contextParams : [])
        ...
        else if (x.toLowerCase() === "subscriptionid" &&
    ...
    if (hasChildClient && client.children) {
        constructor.addStatements(`this._clientParams = {${classicalParams.map((p) => p.name).join(",")}};`);
...
function generateMethod(context, clientType, method) {
    ...
    const methodName = (_b = (_a = declaration.propertyName) !== null && _a !== void 0 ? _a : declaration.name) !== null && _b !== void 0 ? _b : "FIXME";
    const methodParams = (_d = (_c = declaration.parameters) === null || _c === void 0 ? void 0 : _c.filter((p) => p.name !== "context")) !== null && _d !== void 0 ? _d : [];
    ...
    if (((_e = context.rlcOptions) === null || _e === void 0 ? void 0 : _e.compatibilityLro) &&
        (declaration === null || declaration === void 0 ? void 0 : declaration.isLro) &&
        !(declaration === null || declaration === void 0 ? void 0 : declaration.isLroPaging)) {
        ...
        const returnType = (_f = declaration === null || declaration === void 0 ? void 0 : declaration.lroFinalReturnType) !== null && _f !== void 0 ? _f : "void";
    ...
    else if (((_g = context.rlcOptions) === null || _g === void 0 ? void 0 : _g.compatibilityLro) && (declaration === null || declaration === void 0 ? void 0 : declaration.isLroPaging)) {
        const returnType = (_h = declaration === null || declaration === void 0 ? void 0 : declaration.lropagingFinalReturnType) !== null && _h !== void 0 ? _h : "void";
        const pagedAsyncIterableIteratorReference = resolveReference(PagingHelpers.PagedAsyncIterableIterator);
        const beginListAndWaitName = normalizeName(`${getPagingLROMethodName(methodName)}`, NameType.Method);
...
function buildClientOperationGroups(clientMap, dpgContext, clientClass) {
    ...
    for (const [prefixKey, operations] of methodMap) {
        ...
        if (prefixKey === "") {
            ...
            const rawGroupName = normalizeName((_a = prefixes[0]) !== null && _a !== void 0 ? _a : "", NameType.Interface);
            ...
            if (!existProperty || existProperty.length === 0) {
                ...
                (_b = clientClass
                    .getConstructors()[0]) === null || _b === void 0 ? void 0 : _b.addStatements(`this.${groupName} = ${resolveReference(refkey(operationName, layer, "getClassicOperations"))}(this._client)`);
            }
        }
    }
}
function addChildClient(context, clientClass, parentClient, client) {
    ...
    getChildClientFunction.addStatements(`return new ${getClassicalClientName(client)}(
      ${parentParams
...
function hasTenantLevelOperations(client, dpgContext) {
    const methodMap = getMethodHierarchiesMap(dpgContext, client);
...
function generateConstructorWithOverloads(clientClass, classicalParams, client) {
    const filteredParams = classicalParams.filter((p) => p.name.toLowerCase() !== "subscriptionid" &&
...
//# sourceMappingURL=buildClassicalClient.js.map