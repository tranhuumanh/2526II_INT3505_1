...
export function getAvailabilityMapInTimeline(program, type, timeline) {
    ...
    const hasTypeChangedInTimeline = typeChanged !== undefined && [...typeChanged.keys()].some((v) => timeline.getIndex(v) !== -1);
    const hasReturnTypeChangedInTimeline = returnTypeChanged !== undefined &&
        [...returnTypeChanged.keys()].some((v) => timeline.getIndex(v) !== -1);
    if (!timelineAdded.length &&
